...
Sub Load_Data
	Try 
		...
		Cursor = sql1.ExecQuery("select * from ExpensesInfo")
		
		Do While Cursor.NextRow
			
			str = str & "  <tr> "
			str = str & "<td>"& Cursor.GetString("ExpensesID") &"</td>"
			str = str & "<td>"& Cursor.GetString("ExpensesDate") &"</td>"
			str = str & "<td>"& Cursor.GetString("ExpensesAmt") &"</td>"
			
			str = str & "  <td style=' vertical-align: middle !important; white-space: nowrap !important; '> " _
			&" <a href='javascript:void(0);' onclick=""editTbl("& Cursor.GetInt("ExpensesID") &")"" class='btn btn-primary btn-icon mg-r-5 mg-b-10'><div><i class='fa fa-pencil' ></i></div></a> " _
			&" <a href='javascript:void(0);' class='btn btn-danger btn-icon mg-r-5 mg-b-10'><div><i class='fa fa-trash' onclick=""deleteTbl("& Cursor.GetInt("ExpensesID") &")""></i></div></a> " _
			&" </td> </tr>"
...
Private Sub Edit_button_table(params As Map)
	...
	ws.Eval("document.location.replace('AddExpensesInfo.html')",Null)
...
Private Sub Delete_button_table(params As Map)
	...
	sql1.ExecNonQuery("delete from ExpensesInfo where ExpensesID="&params.Get("val"))
