...
Sub Class_Globals
	...
	Private txtinvdate,cmbcustomer,cmbuser,txtinvnote,cmbproduct,txttax,txtunitprice,txtqty,txtdiscount,btnaddproduct,btnsave,btncancel,divproducts,cmbpaymenttype As JQueryElement
...
Sub Load_Customer
	Try
		...
		Do While rs.NextRow
			...
			templistcmbcustomer.Add(rs.GetInt("CustomerID"))
...
Sub Load_PaymentTypes
	Try
		...
		Do While rs.NextRow
			...
			templistpaymenttype.Add(rs.GetInt("PaymenttypeID"))
			
		Loop
		rs.Close
		cmbpaymenttype.SetHtml(sb.ToString)
...
Sub Load_UsersInformation
	Try
		
		Dim sb As StringBuilder
		sb.Initialize
		Dim rs As ResultSet
		rs = sql1.ExecQuery("select * from UsersInformation")
		Do While rs.NextRow
			Dim val As String = rs.GetInt("UserID")
			sb.Append("<option value='").Append(val).Append("'>").Append(rs.GetString("UserFullName")).Append("</option>")
			
		Loop
		rs.Close
		cmbuser.SetHtml(sb.ToString)
...
Sub btnsave_Click (Params As Map)
	Try
		...
		If InvoiceID > 0 Then
				...
				sql1.ExecNonQuery2($"UPDATE SalesInvoiceH
				   SET InvDT = ?
				      ,InvTotal = ?
				      ,UserID = ?
				      ,InvTotalTax = ?
				      ...
				      ,PaymenttypeID = ?
				      ,CustomerID = ?
				 WHERE SalesInvoiceHID=?"$,Array As Object(txtinvdate.GetVal.Value,dblinvtotal,cmbuser.GetVal.Value,dblinvtotaltax _
				 ,dblinvtotaldiscount,0,0,txtinvnote.GetVal.Value,cmbpaymenttype.GetVal.Value,cmbcustomer.GetVal.Value,InvoiceID))
				sql1.TransactionSuccessful
			
				sql1.BeginTransaction
				sql1.ExecNonQuery2("delete from SalesInvoiceD where SalesInvoiceHID=?",Array As Object(InvoiceID))
				sql1.TransactionSuccessful
				...
				For i = 0 To TempProductsTable.Size -1
					...
					sql1.BeginTransaction
					sql1.ExecNonQuery2($"INSERT INTO SalesInvoiceD
			           (SalesInvoiceHID
			           ,ProductID
			           ,ProductSellPrice
			           ,ProductCostPrice
			...
			Else
					...
					sql1.BeginTransaction
					sql1.ExecNonQuery2($"INSERT INTO SalesInvoiceH
				           (InvDT
				           ,InvTotal
				           ,UserID
				           ,InvTotalTax
				     ...
				     VALUES
				           ...
				           ,?
				           ,?
				           ,?)"$,Array As Object(txtinvdate.GetVal.Value,dblinvtotal,cmbuser.GetVal.Value,dblinvtotaltax _
						   ,dblinvtotaldiscount,0,0,txtinvnote.GetVal.Value,cmbpaymenttype.GetVal.Value,cmbcustomer.GetVal.Value))
					sql1.TransactionSuccessful
					
					Dim rs As ResultSet
					rs = sql1.ExecQuery("select SalesInvoiceHID from SalesInvoiceH order by SalesInvoiceHID desc limit 1")
					Do While rs.NextRow
						LastSavedID = rs.GetInt("SalesInvoiceHID")
					Loop
					rs.Close
						...
						For i = 0 To TempProductsTable.Size -1
					     ...
					     VALUES
					           ...
					           ,?
					           ,?
					           ,?)"$,Array As Object(LastSavedID,ClassTempProd.ProductID,ClassTempProd.UnitPrice,ClassTempProd.ProductCostPrice,ClassTempProd.TaxID,ClassTempProd.Qty))
							sql1.TransactionSuccessful
		...
		ws.Eval("document.location.replace('SalesInformation.html')",Null)
...
Sub Load_Data_ByID
	Try
		...
		If InvoiceID > 0 Then
			...
			Dim rs As ResultSet
			rs = sql1.ExecQuery("select * from SalesInvoiceH where SalesInvoiceHID="&InvoiceID)
			Do While rs.NextRow
				
				txtinvdate.SetVal(rs.GetString("InvDT"))
				txtinvnote.SetVal(rs.GetString("StrNotes"))
				
				Dim strID As String = cmbcustomer.id
				strID = strID.Replace("#","")
				ws.RunFunction("setSelectedRow", Array As Object(strID, rs.GetInt("CustomerID")))
				
				
				Dim strID As String = cmbpaymenttype.id
				strID = strID.Replace("#","")
				ws.RunFunction("setSelectedRow", Array As Object(strID, rs.GetInt("PaymenttypeID")))
				
				Dim strID As String = cmbuser.id
				strID = strID.Replace("#","")
				ws.RunFunction("setSelectedRow", Array As Object(strID, rs.GetInt("UserID")))
			...
			Dim rs As ResultSet
			rs = sql1.ExecQuery("select * from SalesInvoiceD where SalesInvoiceHID="&InvoiceID)
			Do While rs.NextRow
				
				Dim ClassTempProd As ClsTempProducts
				ClassTempProd.Initialize
				ClassTempProd.SetValues(rs.GetInt("ProductID"),rs.GetInt("TaxPerc"),rs.GetString("ProductSellPrice"),rs.GetInt("ProductCostPrice"),0)
