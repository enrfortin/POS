...
Sub Load_Data
	Try
		...
		Cursor = sql1.ExecQuery($"SELECT        TransferH.TransferHID, TransferH.InvDT, TransferH.InvTotal, TransferH.FromWarehouseID, TransferH.InvTotalTax, TransferH.InvTotalDiscount, TransferH.InvTotalShipping, TransferH.PaidAmount, 
...
FROM            TransferH LEFT OUTER JOIN
		...
		Do While Cursor.NextRow
			...
			str = str & "  <tr> "
			str = str & "<td>"& Cursor.GetInt("TransferHID") &"</td>"
			str = str & "<td>"& Cursor.GetString("InvDT") &"</td>"
			str = str & "<td>"& Cursor.GetString("ToWarehouseDesc") &"</td>"
			str = str & "<td>"& Cursor.GetString("FromWarehouseDesc") &"</td>"
			str = str & "<td>"& Cursor.GetString("InvTotal") &"</td>"
			
			str = str & "  <td style=' vertical-align: middle !important; white-space: nowrap !important; '> " _
			&" <a href='javascript:void(0);' onclick=""editTbl("& Cursor.GetInt("TransferHID") &")"" class='btn btn-primary btn-icon mg-r-5 mg-b-10'><div><i class='fa fa-pencil' ></i></div></a> " _
			&" <a href='javascript:void(0);' onclick=""deleteTbl("& Cursor.GetInt("TransferHID") &")"" class='btn btn-danger btn-icon mg-r-5 mg-b-10'><div><i class='fa fa-trash' ></i></div></a> " _
			&" </td> </tr>"
...
Private Sub Edit_button_table(params As Map)
	...
	ws.Eval("document.location.replace('AddTransferInfo.html')",Null)
