...
Sub btnsave_Click (Params As Map)
	Try
		...
		If InvoiceID > 0 Then			
				...
				sql1.ExecNonQuery2($"UPDATE PurchaseInvoiceH
				   SET InvDT = ?
				      ,InvTotal = ?
				      ,InvTotalTax = ?
				      ,InvTotalDiscount = ?
				      ...
				      ,StrNotes = ?
				      ,SupplierID = ?
				 WHERE PurchaseInvoiceHID=?"$,Array As Object(txtinvdate.GetVal.Value,dblinvtotal,dblinvtotaltax _
				 ,dblinvtotaldiscount,0,0,txtinvnote.GetVal.Value,cmbsupplier.GetVal.Value,InvoiceID))
				sql1.TransactionSuccessful
			
				sql1.BeginTransaction
				sql1.ExecNonQuery2("delete from PurchaseInvoiceD where PurchaseInvoiceHID=?",Array As Object(InvoiceID))
				sql1.TransactionSuccessful
				...
				For i = 0 To TempProductsTable.Size -1
					...
					sql1.BeginTransaction
					sql1.ExecNonQuery2($"INSERT INTO PurchaseInvoiceD
			           (PurchaseInvoiceHID
			           ,ProductID
			           ,ProductSellPrice
			           ,ProductCostPrice
			...
			Else
					...
					sql1.BeginTransaction
					sql1.ExecNonQuery2($"INSERT INTO PurchaseInvoiceH
				           (InvDT
				           ,InvTotal
				           ,InvTotalTax
				           ,InvTotalDiscount
					...
					Dim rs As ResultSet
					rs = sql1.ExecQuery("select PurchaseInvoiceHID from PurchaseInvoiceH order by PurchaseInvoiceHID desc limit 1")
					Do While rs.NextRow
						LastSavedID = rs.GetInt("PurchaseInvoiceHID")
					Loop
					rs.Close
...
Sub Load_Data_ByID
	Try
		...
		If InvoiceID > 0 Then
			...
			Dim rs As ResultSet
			rs = sql1.ExecQuery("select * from PurchaseInvoiceH where PurchaseInvoiceHID="&InvoiceID)
			Do While rs.NextRow
			...
			Dim rs As ResultSet
			rs = sql1.ExecQuery("select * from PurchaseInvoiceD where PurchaseInvoiceHID="&InvoiceID)
			Do While rs.NextRow
