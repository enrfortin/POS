...
Sub btnsave_Click (Params As Map)
	Try
		...
		If CatID > 0 Then
				...
				sql1.ExecNonQuery2("Update CatInfo set [CatDesc]=?  where CatID=? ", _
				Array As Object(txtname.GetVal.Value,CatID))
				sql1.TransactionSuccessful
			
			Else
				sql1.BeginTransaction
				sql1.ExecNonQuery2("insert into CatInfo (CatDesc) values (?) ", _
				Array As Object(txtname.GetVal.Value))
				sql1.TransactionSuccessful
				
		End If
		
		Main.Insert_UsersLogs(ws,"Categories Information")
		
		
		ws.Eval("document.location.replace('CatsInformation.html')",Null)
...
Sub Load_Data_ByID
	Try
		...
		If CatID > 0 Then
			...
			Dim rs As ResultSet
			rs = sql1.ExecQuery("select * from  CatInfo where CatID="&CatID)
			Do While rs.NextRow
				
				txtname.SetVal(rs.GetString("CatDesc"))
