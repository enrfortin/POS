...
Sub Class_Globals
	...
	Private txtinvdate,cmbsupplier,txtinvnote,cmbproduct,txttax,txtunitprice,txtqty,txtdiscount,btnaddproduct,btnsave,btncancel,divproducts As JQueryElement
...
Sub Load_Customer
	Try
		...
		sb.Initialize
		Dim rs As ResultSet
		rs = sql1.ExecQuery("select * from SuppliersInfo")
		Do While rs.NextRow
			Dim val As String = rs.GetInt("SupplierID")
			sb.Append("<option value='").Append(val).Append("'>").Append(rs.GetString("SupplierName")).Append("</option>")
			
			templistcmbcustomer.Add(rs.GetInt("SupplierID"))
			
		Loop
		rs.Close
		cmbsupplier.SetHtml(sb.ToString)
...
Sub btnsave_Click (Params As Map)
	Try
		...
		If InvoiceID > 0 Then			
				...
				sql1.ExecNonQuery2($"UPDATE ReturnPurchaseInvoiceH
				   SET InvDT = ?
				      ,InvTotal = ?
				      ,InvTotalTax = ?
				      ,InvTotalDiscount = ?
				      ...
				      ,StrNotes = ?
				      ,SupplierID = ?
				 WHERE ReturnPurchaseInvoiceHID=?"$,Array As Object(txtinvdate.GetVal.Value,dblinvtotal,dblinvtotaltax _
				 ,dblinvtotaldiscount,0,0,txtinvnote.GetVal.Value,cmbsupplier.GetVal.Value,InvoiceID))
				sql1.TransactionSuccessful
			
				sql1.BeginTransaction
				sql1.ExecNonQuery2("delete from ReturnPurchaseInvoiceD where ReturnPurchaseInvoiceHID=?",Array As Object(InvoiceID))
				sql1.TransactionSuccessful
				...
				For i = 0 To TempProductsTable.Size -1
					...
					sql1.BeginTransaction
					sql1.ExecNonQuery2($"INSERT INTO ReturnPurchaseInvoiceD
			...
			Else
					...
					sql1.BeginTransaction
					sql1.ExecNonQuery2($"INSERT INTO ReturnPurchaseInvoiceH
				           (InvDT
				           ,InvTotal
				           ,InvTotalTax
				           ,InvTotalDiscount
				     ...
				     VALUES
				           ...
				           ,?
				           ,?
				           ,?)"$,Array As Object(txtinvdate.GetVal.Value,dblinvtotal,dblinvtotaltax _
						   ,dblinvtotaldiscount,0,0,txtinvnote.GetVal.Value,cmbsupplier.GetVal.Value))
					sql1.TransactionSuccessful
					
					Dim rs As ResultSet
					rs = sql1.ExecQuery("select ReturnPurchaseInvoiceHID from ReturnPurchaseInvoiceH order by ReturnPurchaseInvoiceHID desc limit 1")
					Do While rs.NextRow
						LastSavedID = rs.GetInt("ReturnPurchaseInvoiceHID")
					Loop
					rs.Close
		...
		ws.Eval("document.location.replace('PurchaseInformation.html')",Null)
...
Sub Load_Data_ByID
	Try
		...
		If InvoiceID > 0 Then
			...
			Dim rs As ResultSet
			rs = sql1.ExecQuery("select * from ReturnPurchaseInvoiceH where ReturnPurchaseInvoiceHID="&InvoiceID)
			Do While rs.NextRow
				
				txtinvdate.SetVal(rs.GetString("InvDT"))
				txtinvnote.SetVal(rs.GetString("StrNotes"))
				
				Dim strID As String = cmbsupplier.id
				strID = strID.Replace("#","")
				ws.RunFunction("setSelectedRow", Array As Object(strID, rs.GetInt("SupplierID")))
			...
			Dim rs As ResultSet
			rs = sql1.ExecQuery("select * from ReturnPurchaseInvoiceD where ReturnPurchaseInvoiceHID="&InvoiceID)
			Do While rs.NextRow
