...
Sub Load_Data
	Try
		...
		Cursor = sql1.ExecQuery($"SELECT        PurchaseInvoiceH.PurchaseInvoiceHID, PurchaseInvoiceH.InvDT, PurchaseInvoiceH.InvTotal, PurchaseInvoiceH.InvTotalTax, PurchaseInvoiceH.InvTotalDiscount, PurchaseInvoiceH.InvTotalShipping, 
...
FROM            PurchaseInvoiceH LEFT OUTER JOIN
		...
		Do While Cursor.NextRow
			...
			str = str & "  <tr> "
			str = str & "<td>"& Cursor.GetInt("PurchaseInvoiceHID") &"</td>"
			...
			&" <a href='javascript:void(0);' onclick=""editTbl("& Cursor.GetInt("PurchaseInvoiceHID") &")"" class='btn btn-primary btn-icon mg-r-5 mg-b-10'><div><i class='fa fa-pencil' ></i></div></a> " _
			&" <a href='javascript:void(0);' onclick=""deleteTbl("& Cursor.GetInt("PurchaseInvoiceHID") &")"" class='btn btn-danger btn-icon mg-r-5 mg-b-10'><div><i class='fa fa-trash' ></i></div></a> " _
			&" </td> </tr>"
...
Private Sub Edit_button_table(params As Map)
	...
	ws.Eval("document.location.replace('AddPurchaseInfo.html')",Null)
...
Private Sub Delete_button_table(params As Map)
	...
	sql1.ExecNonQuery("delete from PurchaseInvoiceH where PurchaseInvoiceHID="&params.Get("val"))
