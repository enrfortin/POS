...
Sub Load_Data
	Try 
		...
		Cursor = sql1.ExecQuery("select * from CatInfo")
		
		Do While Cursor.NextRow
			
			str = str & "  <tr> "
			str = str & "<td>"& Cursor.GetString("CatID") &"</td>"
			str = str & "<td>"& Cursor.GetString("CatDesc") &"</td>"
			
			str = str & "  <td style=' vertical-align: middle !important; white-space: nowrap !important; '> " _
			&" <a href='javascript:void(0);' onclick=""editTbl("& Cursor.GetInt("CatID") &")"" class='btn btn-primary btn-icon mg-r-5 mg-b-10'><div><i class='fa fa-pencil' ></i></div></a> " _
			&" <a href='javascript:void(0);' class='btn btn-danger btn-icon mg-r-5 mg-b-10'><div><i class='fa fa-trash' onclick=""deleteTbl("& Cursor.GetInt("CatID") &")""></i></div></a> " _
			&" </td> </tr>"
...
Private Sub Edit_button_table(params As Map)
	...
	ws.Eval("document.location.replace('AddCatInfo.html')",Null)
...
Private Sub Delete_button_table(params As Map)
	...
	sql1.ExecNonQuery("delete from CatInfo where CatID="&params.Get("val"))
